...
- File-download endpoints (binary CSV/XLSX/PDF export responses, not JSON) are hit via `window.location.href = '/api/...?...'`, NOT `apiCall` — same authenticated session cookie carries over on a plain GET navigation, no fetch/blob handling needed. Confirmed 2026-08-06 for `/api/applicants/export` and `/api/applicantOrientations/export`.
- `customConfirm(message)` (global helper, single string arg) returns a `Promise<boolean>` — used for a confirm-before-destructive-action prompt in an async click handler: `const ok = await customConfirm('...'); if (!ok) return;`.
- Date-only inputs (`type="date"`) must be formatted as `YYYY-MM-DD` using local date parts (`getFullYear()`/`getMonth()+1`/`getDate()`, manually padded) — never `toISOString().slice(0,10)`, which shifts by the browser's UTC offset and can land on the wrong day. Applies to both writing values into date inputs and computing week-range presets (Monday-Sunday) for export filters.
