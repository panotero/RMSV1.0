...
class TeamController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($parentId && ((int) $parentId === $team->id || self::isWithinSubtree((int) $parentId, $team->id))) {
            return response()->json([
                'success' => false,
    ...
    /**
     * Whether $candidateParentId is $teamId itself or one of its descendants -
     * used to reject a move that would turn the team tree into a cycle.
     */
    private static function isWithinSubtree(int $candidateParentId, int $teamId): bool
    {
        $current = Team::find($candidateParentId);

        while ($current) {
            if ($current->id === $teamId) {
                return true;
            }
            $current = $current->parent_id ? Team::find($current->parent_id) : null;
        }
